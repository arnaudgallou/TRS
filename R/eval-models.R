...
eval_models <- function(files) {
  out <- map(files, function(.x) {
    data <- readr::read_rds(.x)
    loglik_data <- get_jags_sims(data, "loglik")
    matrix <- as.matrix(loglik_data)
    suppressWarnings({
      waic <- get_ic_estimates(loo::waic(matrix))
      r_eff <- loo::relative_eff(exp(matrix), chain_id = data$sims$chain)
      loo <- loo::loo(matrix, r_eff = r_eff)
  ...
  out <- mutate(
    out,
    across(c("waic", "looic"), delta, .names = "delta_{.col}")
  )
