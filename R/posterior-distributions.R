...
#' @examples
#' \dontrun{
#' fetch_jags("path_to_dir", vars = "dtr", elevation_span = 2000) |>
...
plot_posterior_distributions <- function(
  ...
  df_seg <- distinct(data, !!!unlist(facet_dims), .data[[yvar]], .keep_all = TRUE)
  df_seg <- mutate(
    df_seg,
    ymin = as.numeric(factor(.data[[yvar]])),
    ymax = .data$ymax * scale
  )

  plot <- ggplot(data, aes(x, .data[[yvar]], height = y, fill = .data[[yvar]], color = .data[[yvar]]), ...) +
  ...
  colors <- colors %||% rep("#5E8CBA", length(unique(data[[yvar]])))
  plot +
    scale_fill_manual(values = colors) +
    scale_color_manual(values = colorspace::darken(colors, .5, "HLS"))
}
