...
calc_pred_conf <- function(files) {
  out <- map(files, \(file) {
    ...
    out <- purrr::pluck(data, "elev_grad_clim")
...
add_pred_conf <- function(data, fit, formula) {
  ...
  bind_cols(data, rename_with(df_new_fit, to_snake_case))
}

# check_land_type <- function(data) {
#   out <- select(data, starts_with("expl_var"))
#   out <- map_vec(out, \(x) all(x == "land_type"))
#   if (!any(out)) {
