...
#' @examples
#' \dontrun{
#' fetch_jags("path_to_dir", vars = c("dtr", "ts"), elevation_span = 2000) |>
...
plot_regressions_ <- function(plot_data, plot, params, ...) {
...
plot_regressions_.draws <- function(
  ...
  draws <- group_by(plot_data$sims, .data[[params$facet_cols]], .data[[params$facet_rows]])
  ...
  plot + regression_theme(params$n_labels)
}

plot_regressions_.land_types <- function(plot_data, plot, params, ...) {
  plot +
    geom_ribbon(
      aes(
        x = .data$value,
    ...
    scale_color_manual(values = params$colors$ribbons) +
    scale_fill_manual(values = params$colors$ribbons) +
