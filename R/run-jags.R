...
run_jags <- function(
  ...
  if (is_false(default_output)) {
    out <- make_jags_output(x, model$scope, out)
  }
  if (is_false(save)) {
    return(out)
  }
...
make_jags_output <- function(x, scope, fit) {
  ...
  if (scope == "global") {
    ...
    if (length(vars) == 1L) {
      tbl_data <- rename(tbl_data, x = !!rlang::enquo(vars))
    }
  } else {
  ...
  list(
    ...
    summary = tibble(tbl_data_settings, as_tibble(
      ...
      rownames = "parameter",
      .name_repair = to_snake_case # make_clean_names
    )),
...
data_settings <- function(x, expl_vars, scope) {
  out <- tibble(
    tibble::as_tibble_row(expl_vars),
...
jags_array_to_tibble <- function(array) {
  ...
  out <- tidyr::pivot_wider(out, names_from = .data$Var3, values_from = .data$Freq)
