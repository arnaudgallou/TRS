...
walk(dem_folders, \(folder) {
  ...
  folder |>
    ...
    map(\(dem) {
      clim <- terra::crop(bioclim, dem, snap = "out")
      clim <- terra::resample(clim, dem)
      clim <- terra::zonal(clim, dem, fun = "mean", na.rm = TRUE)
      as_tibble(clim)
    }) |>
...
"gis/clim/data/past/var" |>
  ...
  imap(\(raster, name) {
    clim <- map(polygons, \(polygon) {
      out <- terra::extract(raster, polygon, fun = mean)
